...
class SelectionState extends ChangeNotifier {
  ...
  /// 5. Notifies listeners to update the UI
  ///
  /// The new chat inherits the model and permission mode from the worktree's
  /// welcome screen selections, so the user's backend/model choice is
  /// preserved when creating chats via the sidebar "New Chat" button.
  ///
  ...
  Future<void> createChat(
    ...
    // Create the chat data and state
    // Mark as auto-generated so it can be renamed by AI after first response
    ...
    // Apply the worktree's welcome screen model and permission mode
    // so the user's backend selection (e.g. Codex) is preserved.
    chat.setModel(worktree.welcomeModel);
    chat.setPermissionMode(worktree.welcomePermissionMode);

    // Persist the chat (adds to projects.json)
    await restoreService.addChatToWorktree(
      _project.data.repoRoot,
      worktree.data.worktreeRoot,
      chat,
    );

    // Add to the worktree state and select it
