...
mixin _ArchiveMixin on _PersistenceBase {
  ...
  Future<void> restoreArchivedChat({
    ...
    return runWithProjectsIndexLock(() async {
      try {
        ...
        final archivedRef = project.archivedChats
            .where((c) => c.chatId == chatId)
            .firstOrNull;
        if (archivedRef == null) {
          developer.log(
        ...
        final updatedArchived = project.archivedChats
            .where((c) => c.chatId != chatId)
            .toList();
  ...
  Future<void> archiveWorktreeChats({
    ...
    return runWithProjectsIndexLock(() async {
      ...
      final archivedRefs = worktree.chats
          .map(
            (chatRef) => ArchivedChatReference.fromChatReference(
              chatRef,
              worktreePath: worktreePath,
            ),
          )
  ...
  Future<void> deleteArchivedChat({
    ...
    try {
      await runWithProjectsIndexLock(() async {
        final projectsIndex = await loadProjectsIndex();
        final project = projectsIndex.projects[projectRoot];
