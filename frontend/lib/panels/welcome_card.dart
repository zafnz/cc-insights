...
class WelcomeCard extends StatelessWidget {
    ...
    return Column(
      children: [
        ...
        MessageInput(
          ...
          onSubmit: (text, images, displayFormat) => _createChatAndSendMessage(
            context,
            worktree,
            text,
            images,
  ...
  static Future<void> _createChatAndSendMessage(
    ...
    // Create a new chat in the worktree with agent ID
    final chat = Chat.create(
      name: chatName,
      worktreeRoot: worktree.data.worktreeRoot,
      isAutoGeneratedName: isAutoGenerated,
      agentId: worktree.welcomeAgentId,
    );

    // Apply the selected model, security config, and reasoning effort
    // from the worktree's welcome screen state
    chat.settings.setModel(worktree.welcomeModel);
    chat.settings.setSecurityConfig(worktree.welcomeSecurityConfig);
    chat.settings.setReasoningEffort(worktree.welcomeReasoningEffort);
    ...
    chat.conversations.addEntry(userEntry);
