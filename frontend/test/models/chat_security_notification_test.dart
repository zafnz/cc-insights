...
void main() {
  group('Chat - Security Configuration Notifications', () {
    test('changing Codex sandbox mode adds system notification', () {
      // Arrange
      final chat = Chat.create(name: 'Test Chat', worktreeRoot: '/test');
    ...
    test(
      'changing both sandbox mode and approval policy adds combined notification',
      () {
        // Arrange
        final chat = Chat.create(name: 'Test Chat', worktreeRoot: '/test');
        chat.settings.setSecurityConfig(
        ...
        check(lastEntry).isA<SystemNotificationEntry>();
        final notification = lastEntry as SystemNotificationEntry;
        check(notification.message).equals(
          'Sandbox changed to Full Access. Approval policy set to Never',
        );
      },
    );
    ...
    test('changing Claude permission mode adds system notification', () {
      ...
      final chat = Chat.create(name: 'Test Chat', worktreeRoot: '/test');
      chat.settings.setSecurityConfig(
        const sdk.ClaudeSecurityConfig(
      ...
      ).equals('Permission mode changed to acceptEdits');
    });
