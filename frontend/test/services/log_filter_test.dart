...
void main() {
  ...
  group('LogFilter.parse', () {
    ...
    test('handles single-quoted values', () {
      final filter = LogFilter.parse(".source == 'App'");
      expect(filter, isNotNull);
      expect(filter!.matches(entry(source: 'App')), isTrue);
    });
  ...
  group('LogFilter.matches — eq operator', () {
    test('matches source field', () {
      final filter = LogFilter.parse('.source == "App"')!;
      expect(filter.matches(entry(source: 'App')), isTrue);
      expect(filter.matches(entry(source: 'Flutter')), isFalse);
    });
    ...
    test('matches message field', () {
      ...
      expect(filter.matches(entry(message: 'hello world')), isTrue);
      expect(filter.matches(entry(message: 'other')), isFalse);
    });

    test('matches meta field', () {
      final filter = LogFilter.parse('.meta.worktree == "main"')!;
      expect(
        filter.matches(entry(meta: {'worktree': 'main'})),
        isTrue,
      );
      expect(
        filter.matches(entry(meta: {'worktree': 'feature'})),
        isFalse,
      );
      expect(filter.matches(entry()), isFalse); // no meta
    });

    test('matches nested meta fields', () {
      final filter = LogFilter.parse('.meta.context.id == "123"')!;
      expect(
        filter.matches(entry(meta: {
          'context': {'id': '123'},
        })),
        isTrue,
      );
  ...
  group('LogFilter.matches — neq operator', () {
    ...
    test('matches when field is missing (meta)', () {
      final filter = LogFilter.parse('.meta.worktree != "main"')!;
      expect(filter.matches(entry()), isTrue); // no meta = not equal
    });
  });
  ...
  group('LogFilter.matches — contains operator', () {
    test('finds substring in message', () {
      ...
      expect(filter.matches(entry(message: 'hello world')), isTrue);
      expect(filter.matches(entry(message: 'hello')), isFalse);
    });
    ...
    test('works on source field', () {
      ...
      expect(filter.matches(entry(source: 'Flutter')), isTrue);
      expect(filter.matches(entry(source: 'App')), isFalse);
    });
  });
