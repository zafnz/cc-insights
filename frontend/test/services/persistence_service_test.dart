  ...
  group('PersistenceService', () {
    ...
    group('loadCostTracking', () {
      ...
      test('reads single entry correctly', () async {
        ...
        await persistence.appendCostTracking(projectId, entry);
        ...
        check(result[0].worktree).equals('main');
        check(result[0].chatName).equals('Test Chat');
        check(result[0].timestamp).equals('2024-01-15T10:30:00Z');
        check(result[0].backend).equals('claude');
        check(result[0].modelUsage.length).equals(1);
        check(result[0].modelUsage[0].modelName).equals('claude-opus-4');
        check(result[0].modelUsage[0].inputTokens).equals(1000);
        check(result[0].modelUsage[0].outputTokens).equals(500);
        check(result[0].modelUsage[0].costUsd).equals(0.0123);
      });
      ...
      test('reads multiple entries', () async {
        ...
        // Assert
        check(result.length).equals(2);
        check(result[0].chatName).equals('Chat 1');
        check(result[1].chatName).equals('Chat 2');
        check(result[0].worktree).equals('main');
        check(result[1].worktree).equals('feature');
      });
      ...
      test('skips invalid JSON lines gracefully', () async {
        ...
        const projectId = 'test-project';
        final trackingPath =
            persistence.costTrackingPath(projectId);
        final file = File(trackingPath);
        await file.parent.create(recursive: true);

        await file.writeAsString(
          '{"worktree":"main","chatName":"Valid","timestamp":"2024-01-15T10:00:00Z","modelUsage":[],"backend":"claude"}\n'
          'invalid json line\n'
          '{"worktree":"feature","chatName":"Also Valid","timestamp":"2024-01-15T11:00:00Z","modelUsage":[],"backend":"claude"}\n',
        );

        // Act
        final result = await persistence.loadCostTracking(projectId);

        // Assert - should load 2 valid entries and skip the invalid one
        check(result.length).equals(2);
        check(result[0].chatName).equals('Valid');
        check(result[1].chatName).equals('Also Valid');
      });
      ...
      test('preserves backend field from entries', () async {
        ...
        check(result.length).equals(2);
        check(result[0].backend).equals('claude');
        check(result[1].backend).equals('codex');
      });

      test('round-trip: appendCostTracking then loadCostTracking returns same data',
          () async {
        // Arrange
        const projectId = 'test-project';
        ...
        // Act
        await persistence.appendCostTracking(projectId, originalEntry);
        final result = await persistence.loadCostTracking(projectId);

        // Assert
        check(result.length).equals(1);
        final loadedEntry = result[0];
        check(loadedEntry.worktree).equals(originalEntry.worktree);
        check(loadedEntry.chatName).equals(originalEntry.chatName);
        check(loadedEntry.timestamp).equals(originalEntry.timestamp);
        check(loadedEntry.backend).equals(originalEntry.backend);
        check(loadedEntry.modelUsage.length)
            .equals(originalEntry.modelUsage.length);

        for (var i = 0; i < originalEntry.modelUsage.length; i++) {
          final original = originalEntry.modelUsage[i];
          final loaded = loadedEntry.modelUsage[i];
          check(loaded.modelName).equals(original.modelName);
          check(loaded.inputTokens).equals(original.inputTokens);
          check(loaded.outputTokens).equals(original.outputTokens);
          check(loaded.cacheReadTokens).equals(original.cacheReadTokens);
          check(loaded.cacheCreationTokens).equals(original.cacheCreationTokens);
          check(loaded.costUsd).equals(original.costUsd);
          check(loaded.contextWindow).equals(original.contextWindow);
        }
      });
    });
  });
...
class _TestPersistenceService extends PersistenceService {
  ...
  String costTrackingPath(String projectId) =>
      '$_baseDir/projects/$projectId/tracking.jsonl';
